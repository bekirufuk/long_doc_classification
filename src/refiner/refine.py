...
def main():
    ...
    for file_index, file_dir in enumerate(files):
        ...
        for chunk_index, chunk in enumerate(chunks):
            ...
            print(f"================ FILE {patent_year} CHUNK {chunk_index} ============ Total Records:{record_count}, {short_record_count}")

            # Log the step to the refiner_log.txt
            log_text = datetime.datetime.now().strftime("%Y-%m-%d-%H:%M")
            log_text += "->Year {} Chunk {} saved containing {}, {} records".format(patent_year, chunk_index, chunk_len,short_record_count)
            log_text += "Running record count for this merge: {}, {}".format(record_count, short_record_count)
