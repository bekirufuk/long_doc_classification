...
if __name__ == '__main__':
    ...
    tfidf_sparse = pd.read_pickle('data/refined_patents/tfidf/big_bird/train_tfidf_sparse.pkl')
    f_names = pd.read_pickle('data/refined_patents/tfidf/big_bird/train_f_list.pkl')
    ...
    for epoch in range(finetune_config['epochs']):
        for batch_id, batch in enumerate(train_dataloader):
            ...
            is_log_step = step_counter % log_interval == 0 and step_counter!=1
            ...
            if is_log_step:
                outputs = model(batch['input_ids'], 
                            ...
                            special_random_blocks = tfidf_batch_map
                            )
                ...
                if mean_train_accuracy > best_train_accuracy:
                    ...
                    model.save_pretrained('models/{model_type}/{model}_best_accuracy'.format(model_type=finetune_config['model_type'], model=finetune_config['model']))
                    #print('Model Checkpoint with {} accuracy at step {}'.format(str(best_train_accuracy), str(step_counter-1) ))
    ...
    model.save_pretrained('models/{model_type}/{model}_final'.format(model_type=finetune_config['model_type'], model=finetune_config['model']))
    print("\n----------\n TRAINING FINISHED \n----------\n")
    ...
    model = BigBirdForSequenceClassification.from_pretrained('models/{model_type}/{model}_final'.format(model_type=finetune_config['model_type'], model=finetune_config['model']))
