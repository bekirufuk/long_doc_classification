...
from transformers import BertForSequenceClassification, BertConfig, get_cosine_schedule_with_warmup

sys.path.append(os.getcwd())
from src.data_processer.process import get_bert_tokens
...
if __name__ == '__main__':
    ...
    train_data = get_bert_tokens(load_tokens=True, test_data_only=False, train_sample_size=finetune_config['train_sample_size'])
    ...
    bert_config = BertConfig.from_json_file('src/config/model_configs/bert.json')
    ...
    # Load the best model.
    model = BertForSequenceClassification.from_pretrained('models/{model_type}/{model}'.format(model_type=finetune_config['model_type'], model=finetune_config['model']))

    test_data = get_bert_tokens(load_tokens=True, test_data_only=True, test_sample_size=finetune_config['test_sample_size'])
