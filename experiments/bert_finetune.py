...
from transformers import BertTokenizerFast, BertForSequenceClassification, BertConfig, get_scheduler, get_cosine_schedule_with_warmup
...
def get_tokenized_data():
    if config.load_saved_tokens:
        ...
        tokenized_data = load_from_disk(os.path.join(config.data_dir, "bert_tokenized/"))
    else:
        ...
        tokenizer = BertTokenizerFast.from_pretrained('bert-base-uncased', max_length=config.max_length)
...
def save_tokenized_data():
    ...
    tokenized_data.save_to_disk(os.path.join(config.data_dir, "bert_tokenized/"))

def get_model():
    if config.load_local_checkpoint:
        model = BertForSequenceClassification.from_pretrained(os.path.join(config.root_dir,"models/{}".format(config.model_name)))
    else:
        # Utilize the model with custom config file specifiyng classification labels.
        bigbird_config = BertConfig.from_json_file(os.path.join(config.root_dir, 'bert_config.json'))
